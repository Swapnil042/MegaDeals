    ...
    <script>
            ...
            function deleterow(id) {
                if (window.confirm('Do you want to delete the data?')) {
                    window.location.assign('laptop.php?status=connect&type=delete&l_id=' + id);
                }
            }
      ...
      <section class="wrapper">
        ...
        <div>
            ...
            <span id="left_menu" style="width:20%;height:600px;overflow:auto;background-color:#ffddcc;display:inline-block;">
                <input id="cdb" name="cdb" type="button" value="Connect DataBase" style="width:100%;" onclick="window.location.assign('laptop.php?status=connect');"><br/>
                <input id="ddb" name="ddb" type="button" value="Disconnect DataBase" style="width:100%;" disabled onclick="window.location.assign('laptop.php?status=disconnect')"><br/>
            </span>
            ...
            <span id="content" style="width:78%;height:600px;overflow:auto;display:inline-block;background-color:#ecf9f2;">
                <div id="insertsection" style="width:99%;max-height:400px;display:none;">
                    <h4>Insert Data For Laptop</h4>
                    <form method='get' action='laptop.php'>
                        p_name: <input type='text' name='p_name' id='p_name'> 
                        Processor: <input type='text' name='proc' id='proc'>
                        Os: <input type='text' name='os' id='os'>
                        <hr>
                        Graphics: <input type='text' name='grap' id='grap'>
                        Memory: <input type='text' name='mem' id='mem'>
                        Storage: <input type='text' name='stor' id='stor'>
                        <hr>
                        Camera: <input type='text' name='camera' id='camera'>
                        Battery: <input type='text' name='battr' id='battr'>
                        Display: <input type='text' name='disp' id='disp'>
                        <hr>
                        Touch_screen: <input type='text' name='touch' id='touch'>
                        Audio: <input type='text' name='audio' id='audio'>
                        Port: <input type='text' name='port' id='port'>
                        <hr>
                        Connectivity: <input type='text' name='con' id='con'>
                        Dimensions: <input type='text' name='dim' id='dim'>
                ...
                if($status=='connect'){
                    ...
                    try {
                        ...
                        if(isset($_GET['type'])&& $_GET['type']=='insert'){
                            $flag=true;
                            try {
                                $pdostmt = $conn->prepare("select max(l_id) as total from laptop_info");
                                $pdostmt->execute();
                                ...
                                if (isset($_GET['p_name']) && isset($_GET['proc']) && isset($_GET['os']) && isset($_GET['grap']) && isset($_GET['mem']) && isset($_GET['stor']) && isset($_GET['camera']) && isset($_GET['battr']) && isset($_GET['disp']) && isset($_GET['touch']) && isset($_GET['audio']) && isset($_GET['port']) && isset($_GET['con']) && isset($_GET['dim']) && isset($_GET['wght']) && isset($_GET['price']) && isset($_GET['img'])){ 
                                    $stmt = "insert into laptop_info values(" . ($len + 1) . ",'" . $_GET['p_name'] . "','" . $_GET['proc'] . "','" . $_GET['os'] . "','" . $_GET['grap'] . "','" . $_GET['mem'] . "','" . $_GET['stor'] . "','" . $_GET['camera'] . "','" . $_GET['battr'] . "','" . $_GET['disp'] . "','" . $_GET['touch'] . "','" . $_GET['audio'] . "','" . $_GET['port'] . "','" . $_GET['con'] . "','" . $_GET['dim'] . "','" . $_GET['wght'] . "','" . $_GET['price'] . "','" . $_GET['img'] . "')";
                                    $conn->exec($stmt);
                                }
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='delete'){
                            ...
                            try {
                                if (isset($_GET['l_id'])) {
                                    $stmt = 'delete from laptop_info where l_id=' . $_GET['l_id'];
                                    $conn->exec($stmt);
                                }
                            } catch (PDOException $ex) {
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='update'){
                            ...
                            try {
                                    if (isset($_GET['l_id'])) {
                                    $pdostat = $conn->prepare("select * from laptop_info where l_id='". $_GET['l_id']."'");
                                    $pdostat->execute();
                       ...
                       ///when order details parameter is passed with id
                        ...
                        try {
                            $pdostat = $conn->prepare("select * from laptop_info ");
                            $pdostat->execute();
                            $res = $pdostat->fetchAll(PDO::FETCH_NUM);
                            $tablegenerate = "<h4>Show Table Data</h4>  <table style='width:100%;border-collapse:collapse;border:1px solid black;'><thead><tr><th>l_id</th><th>p_name</th><th>Processor</th><th>Os</th><th>Graphics</th><th>Memory</th><th>Storage</th><th>Camera</th><th>Battery</th><th>Display</th><th>Touch_screen</th><th>Audio</th><th>Port</th><th>Connectivity</th><th>Dimensions</th><th>Weight</th><th>Price</th><th>Img</th></tr></thead></tbody>";
                            foreach ($res as $innarr) {
