    ...
    <script>
            ...
            function deleterow(id) {
                if (window.confirm('Do you want to delete the data?')) {
                    window.location.assign('camera.php?status=connect&type=delete&c_id=' + id);
                }
            }
      ...
      <section class="wrapper">
        ...
        <div>
            ...
            <span id="left_menu" style="width:20%;height:600px;overflow:auto;background-color:#ffddcc;display:inline-block;">
                <input id="cdb" name="cdb" type="button" value="Connect DataBase" style="width:100%;" onclick="window.location.assign('camera.php?status=connect');"><br/>
                <input id="ddb" name="ddb" type="button" value="Disconnect DataBase" style="width:100%;" disabled onclick="window.location.assign('camera.php?status=disconnect')"><br/>
            </span>
            ...
            <span id="content" style="width:78%;height:600px;overflow:auto;display:inline-block;background-color:#ecf9f2;">
                <div id="insertsection" style="width:99%;max-height:400px;display:none;">
                    <h4>Insert Data For Camera</h4>
                    <form method='get' action='camera.php'>
                        c_Name: <input type='text' name='c_name' id='c_name'> 
                        lens_mount: <input type='text' name='lens' id='lens'>
                        total_pixel: <input type='text' name='total_p' id='total_p'>
                        <hr>
                        effective_pixel: <input type='text' name='effctv_p' id='effctv_p'>
                        img_quality: <input type='text' name='img_q' id='img_q'>
                        white_balance: <input type='text' name='white_b' id='white_b'>
                        <hr>
                        aspect_ratio: <input type='text' name='aspct_r' id='aspct_r'>
                        photo_4k: <input type='text' name='pht_4' id='pht_4'>
                        focus: <input type='text' name='fcus' id='fcus'>
                        <hr>
                        battery: <input type='text' name='bttr' id='bttr'>
                        dimension: <input type='text' name='dimn' id='dimn'>
                        weight: <input type='text' name='wght' id='wght'>
                        <hr>
                        connectivity: <input type='text' name='con' id='con'>
                ...
                if($status=='connect'){
                    ...
                    try {
                        ...
                        if(isset($_GET['type'])&& $_GET['type']=='insert'){
                            $flag=true;
                            try {
                                $pdostmt = $conn->prepare("select max(c_id) as total from camera_info");
                                $pdostmt->execute();
                                ...
                                if (isset($_GET['c_name']) && isset($_GET['lens']) && isset($_GET['total_p']) && isset($_GET['effctv_p']) && isset($_GET['img_q']) && isset($_GET['white_b']) && isset($_GET['aspct_r']) && isset($_GET['pht_4']) && isset($_GET['fcus']) && isset($_GET['bttr']) && isset($_GET['dimn']) && isset($_GET['wght']) && isset($_GET['con']) && isset($_GET['price']) && isset($_GET['img'])){ 
                                    $stmt = "insert into camera_info values(" . ($len + 1) . ",'" . $_GET['c_name'] . "','" . $_GET['lens'] . "','" . $_GET['total_p'] . "','" . $_GET['effctv_p'] . "','" . $_GET['img_q'] . "','" . $_GET['white_b'] . "','" . $_GET['aspct_r'] . "','" . $_GET['pht_4'] . "','" . $_GET['fcus'] . "','" . $_GET['bttr'] . "','" . $_GET['dimn'] . "','" . $_GET['wght'] . "','" . $_GET['con'] . "','" . $_GET['price'] . "','" . $_GET['img'] . "')";
                                    $conn->exec($stmt);
                                }
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='delete'){
                            ...
                            try {
                                if (isset($_GET['c_id'])) {
                                    $stmt = 'delete from camera_info where c_id=' . $_GET['c_id'];
                                    $conn->exec($stmt);
                                }
                            } catch (PDOException $ex) {
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='update'){
                            ...
                            try {
                                    if (isset($_GET['c_id'])) {
                                    $pdostat = $conn->prepare("select * from camera_info where c_id='". $_GET['c_id']."'");
                                    $pdostat->execute();
                       ...
                       ///when order details parameter is passed with id
                        ...
                        try {
                            $pdostat = $conn->prepare("select * from camera_info");
                            $pdostat->execute();
                            $res = $pdostat->fetchAll(PDO::FETCH_NUM);
                            $tablegenerate = "<h4>Show Table Data</h4>  <table style='width:100%;border-collapse:collapse;border:1px solid black;'><thead><tr><th>c_id</th><th>c_Name</th><th>lens_mount</th><th>total_pixel</th><th>effective_pixel</th><th>img_quality</th><th>white_balance</th><th>aspect_ratio</th><th>photo_4k</th><th>focus</th><th>battery</th><th>dimension</th><th>weight</th><th>connectivity</th><th>price</th><th>Img</th></tr></thead></tbody>";
                            foreach ($res as $innarr) {
