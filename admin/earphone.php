    ...
    <script>
            ...
            function deleterow(id) {
                if (window.confirm('Do you want to delete the data?')) {
                    window.location.assign('earphone.php?status=connect&type=delete&e_id=' + id);
                }
            }
      ...
      <section class="wrapper">
        ...
        <div>
            ...
            <span id="left_menu" style="width:20%;height:600px;overflow:auto;background-color:#ffddcc;display:inline-block;">
                <input id="cdb" name="cdb" type="button" value="Connect DataBase" style="width:100%;" onclick="window.location.assign('earphone.php?status=connect');"><br/>
                <input id="ddb" name="ddb" type="button" value="Disconnect DataBase" style="width:100%;" disabled onclick="window.location.assign('earphone.php?status=disconnect')"><br/>
            </span>
            ...
            <span id="content" style="width:78%;height:600px;overflow:auto;display:inline-block;background-color:#ecf9f2;">
                <div id="insertsection" style="width:99%;max-height:400px;display:none;">
                    <h4>Insert Data For Earphone</h4>
                    <form method='get' action='earphone.php'>
                        p_name: <input type='text' name='p_name' id='p_name'> 
                        Colour: <input type='text' name='clor' id='clor'>
                        Type: <input type='text' name='type' id='type'>
                        <hr>
                        Frequency: <input type='text' name='freq' id='freq'>
                        Accessories: <input type='text' name='accs' id='accs'>
                        Warranty: <input type='text' name='warn' id='warn'>
                        <hr>
                ...
                if($status=='connect'){
                    ...
                    try {
                        ...
                        if(isset($_GET['type'])&& $_GET['type']=='insert'){
                            $flag=true;
                            try {
                                $pdostmt = $conn->prepare("select max(e_id) as total from earphone_info");
                                $pdostmt->execute();
                                ...
                                if (isset($_GET['p_name']) && isset($_GET['clor']) && isset($_GET['type']) && isset($_GET['freq']) && isset($_GET['accs']) && isset($_GET['warn']) && isset($_GET['audio']) && isset($_GET['blth']) && isset($_GET['price']) && isset($_GET['img'])){ 
                                    $stmt = "insert into earphone_info values(" . ($len + 1) . ",'" . $_GET['p_name'] . "','" . $_GET['clor'] . "','" . $_GET['type'] . "','" . $_GET['freq'] . "','" . $_GET['accs'] . "','" . $_GET['warn'] . "','" . $_GET['audio'] . "','" . $_GET['blth'] . "','" . $_GET['price'] . "','" . $_GET['img'] . "')";
                                    $conn->exec($stmt);
                                }
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='delete'){
                            ...
                            try {
                                if (isset($_GET['e_id'])) {
                                    $stmt = 'delete from earphone_info where e_id=' . $_GET['e_id'];
                                    $conn->exec($stmt);
                                }
                            } catch (PDOException $ex) {
                        ...
                        if(isset($_GET['type']) && $_GET['type']=='update'){
                            ...
                            try {
                                    if (isset($_GET['e_id'])) {
                                    $pdostat = $conn->prepare("select * from earphone_info where e_id='". $_GET['e_id']."'");
                                    $pdostat->execute();
                       ...
                       ///when order details parameter is passed with id
                        ...
                        try {
                            $pdostat = $conn->prepare("select * from earphone_info");
                            $pdostat->execute();
                            $res = $pdostat->fetchAll(PDO::FETCH_NUM);
                            $tablegenerate = "<h4>Show Table Data</h4>  <table style='width:100%;border-collapse:collapse;border:1px solid black;'><thead><tr><th>e_id</th><th>p_name</th><th>Colour</th><th>Type</th><th>Frequency</th><th>Accessories</th><th>Warranty</th><th>Audio</th><th>Bluetooth</th><th>Price</th><th>Img</th></tr></thead></tbody>";
                            foreach ($res as $innarr) {
